...
export const generateWAMessageContent = async (
  ...
  if ("text" in message) {
    ...
    if (contactLen === 1) {
      m.contactMessage = WAProto.Message.ContactMessage.fromObject(
        message.contacts.contacts[0]
      );
    } else {
      m.contactsArrayMessage = WAProto.Message.ContactsArrayMessage.fromObject(
        message.contacts
      );
    }
  } else if ("location" in message) {
    m.locationMessage = WAProto.Message.LocationMessage.fromObject(
      message.location
    );
...
export const getDevice = (id: string) => {
  const deviceType =
    id.length > 21 ? "android" : id.substring(0, 2) === "3A" ? "ios" : "web";
  return deviceType;
};

/** Upserts a receipt in the message */
export const updateMessageWithReceipt = (
...
export const updateMessageWithReaction = (
